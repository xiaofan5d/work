...
import pandas as pd
import pymssql
#conn = pymssql.connect("192.168.8.238:1433", "sa", "Mp123456", "Mentholatum2018")
...
def load_data():
    ...
    messagebox.showinfo(title='导入数据', message='请导入厂价表数据')
    print('\033[1;32m 请导入厂价表 \033[0m')
    sku_path = filedialog.askopenfilename()
    ...
    sku = pd.read_excel(sku_path).fillna(0)
    ...
    if len(stock_QA_sanhuo)>0:
        Outbound_order_QA_sanhuo,Order_NR_sanhuo=oo.stock_cl_paixu(stock_QA_sanhuo, Order_QA_sanhuo,Order_column)
    else:
    ...
    if len(stock_NR_sanhuo)>0:
        Outbound_order_NR_sanhuo, Order_QA_zhengxiang = oo.stock_cl_paixu(stock_NR_sanhuo, Order_NR_sanhuo,Order_column)
    else:
    ...
    if len(stock_QA_zhengxiang)>0:
        Outbound_order_QA_zhengxiang, Order_NR_zhengxiang = oo.stock_cl_paixu(stock_QA_zhengxiang, Order_QA_zhengxiang, Order_column, method='zhengxiang')
    else:
        print('\033[1;31m 提示:QA整箱跳过 \033[0m')
        Outbound_order_QA_zhengxiang= pd.DataFrame()
        Order_NR_zhengxiang=Order_QA_zhengxiang
    Outbound_order_NR_zhengxiang,Order_all=oo.stock_cl_paixu(stock_NR_zhengxiang, Order_NR_zhengxiang, Order_column,method='zhengxiang')
...
B = tk.Button(root, text="点击导入数据并导出", command=load_data)
B.pack()
root.mainloop()
