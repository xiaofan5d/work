...
path_订单 = '3-20订单'
path_库存 = '库存0320'
Order = pd.read_excel('D://work//曼秀雷敦//出库单发仓库//' + path_订单 + '.xls')
stock = pd.read_excel(
    'D://work//曼秀雷敦//库存//' +
    path_库存 +
    '.xlsx',
...
#%%
订单_column = Order.columns
订单_column2 = list(订单_column)
订单_column2.append('采购数量1')
订单_column2.append('箱规')
#%%
#判断是否订单中的SKU厂价表中已维护
isin_sku(Order,sku)
#处理库存(散货以及整箱)
stock=stock_cl(stock,sku,Order)
#处理订单(搞上箱规)(以及判断箱规是否合理)
Order=Order_cl(stock,Order,sku)
#%%
Order['采购数量1'] = Order['采购数量']
Order_zero=Order.loc[Order.采购数量==0]
Order_QA_sanhuo=Order.loc[Order.采购数量!=0]
...
def stock_cl_paixu(stock,Order,method):
    #订单匹配库存数量
    ...
    订单_bak = Order.reindex(columns=订单_column2)
    if method=='sanhuo':
        print('散货分配')
    elif method=='zhengxiang':
...
#%%
Outbound_order_QA_sanhuo,Order_QA_zhengxiang=stock_cl_paixu(stock_QA_sanhuo, Order_QA_sanhuo, method='sanhuo')
#%%
Outbound_order_QA_zhengxiang,Order_NR_sanhuo=stock_cl_paixu(stock_QA_zhengxiang, Order_QA_zhengxiang, method='zhengxiang')
#%%
Outbound_order_NR_sanhuo,Order_NR_zhengxiang=stock_cl_paixu(stock_NR_sanhuo, Order_NR_sanhuo, method='sanhuo')
#%%
Outbound_order_NR_zhengxiang,Order_all=stock_cl_paixu(stock_NR_zhengxiang, Order_NR_zhengxiang, method='zhengxiang')
#%%
data=Outbound_order_QA_sanhuo.append(Outbound_order_QA_zhengxiang).append(Outbound_order_NR_sanhuo).append(Outbound_order_NR_zhengxiang)

# 分货完原订单数量减去-分货数量汇总生成新订单
#%%
...
导出路径 = pd.ExcelWriter('D://work//曼秀雷敦//test出库单发仓库出库明细@' + path_库存 + '(发仓库).xlsx')
