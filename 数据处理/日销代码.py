...
engine = create_engine('mssql+pymssql://sa:Mp123456@192.168.8.238:1433/jingdong_salls',echo=True)  # 建立数据库连接
...
class MSSQL:
    ...
    def ExecNonQuery(self,sql):
        ...
        self.conn.close()
def delete():
    ms=MSSQL("192.168.8.238:1433", "sa", "Mp123456", "jingdong_salls")
    back_val=ms.ExecNonQuery("DELETE FROM temp_rixiao_cl")  #注意，这里back_val为None值。
    return back_val
delete()
pd.io.sql.to_sql(data,'temp_rixiao_cl',con=engine,if_exists='append',index=False)
engine.dispose()
